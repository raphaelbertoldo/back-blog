...
class PersonService {
  ...
  async store(name, email, age, picture, password) {
    try {
      const id = uuidv4();
      const hash = await bcrypt.hash(password, 10);
      await this.db.run(
        `Create (u:Person {_id: "${id}", name: "${name}", age: "${age}", email: "${email}", picture: "${picture}", password: "${hash}", createdAt: "datetime(${new Date()})"} ) return u`
      );
